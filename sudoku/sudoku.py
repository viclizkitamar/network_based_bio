# after getting result to add it as a constraint for the next iteration -that i don't want this solution

#to compare that this solve is different . not 1:1

#to think about the reduction sudoku-> sat-> 3 sat -> subset sum

#to call different solver 
...
def solve(grid):
    ...
    clauses = sudoku_clauses() #--> in general all the clauses
    for i in range(1, 10):
        for j in range(1, 10):
            d = grid[i - 1][j - 1]
    ...
    for sol2 in pycosat.itersolve(clauses):
    
		# solve the SAT problem
        start = time.time()
		#pprint("those are the clauses: {}".format(clauses)) #sending it as a dimacs
		
        sol = set(sol2)
		#sol = set(pycosat.solve(clauses))
		
		#nonreturners=list(itertools.islice(pycosat.itersolve(clauses),2))
		# len(list(pycosat.itersolve(clauses))) ----> to know how many solutions we have.
		...
		#pprint("this is the first solution: {}".format(nonreturners[1]))
